...
describe('sanitizeProvider', () => {
  it('removes internal fields and replaces tilemapUrl', () => {
    ...
    expect(sanitized).to.have.property('tilemapUrl').that.includes('/signalk/chart-tiles');
  });
  ...
  it('handles version 2 and vector format', () => {
    ...
    expect(sanitized).to.have.property('url').that.includes('/signalk/chart-style/vec');
    expect(sanitized).to.have.property('style').that.includes('/signalk/chart-style/vec');
  });
});
