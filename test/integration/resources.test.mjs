...
describe('Charts API & Tile Provider', () => {
  ...
  describe('GET /signalk/v1/api/resources/charts', () => {
    ...
    it('returns all charts for multiple paths', () => {
      return pluginInstance.start({ // Fixed: pluginInstance instead of plugin
        chartPaths: [
          path.resolve(fixturesRoot, 'mbtiles'),
          path.resolve(fixturesRoot, 'directory'),
          path.resolve(fixturesRoot, 'tms')
    ...
    it('returns empty charts for custom empty path', () => {
      ...
      return pluginInstance.start({ chartPaths: [emptyPath] })
      .then(() => get(testServer, '/signalk/v1/api/resources/charts'))
      .then(result => {
    ...
    it('returns one chart', () => {
      ...
      return pluginInstance.start({ chartPaths: [path.resolve(fixturesRoot, 'mbtiles')] })
  ...
  describe('GET /signalk/chart-tiles/:identifier/:z/:x/:y', () => {
    it('returns correct tile from MBTiles file', () => {
      ...
      .then(response => {
        expectTileResponse(response, 'directory/unpacked-tiles/4/5/6.png', 'image/png')
      })
    })

    it('returns correct tile from directory', () => {
      return pluginInstance.start({ chartPaths: [path.resolve(fixturesRoot, 'directory')] })
      .then(() => get(testServer, '/signalk/chart-tiles/unpacked-tiles/4/4/6'))
      .then(response => {
        expectTileResponse(response, 'directory/unpacked-tiles/4/4/6.png', 'image/png')
      })
    })

    it('returns correct tile from TMS directory', () => {
      return pluginInstance.start({ chartPaths: [path.resolve(fixturesRoot, 'tms')] })
...
const expectTileResponse = (response, expectedTilePath, expectedFormat) => {
  const expectedTile = fs.readFileSync(path.resolve(fixturesRoot, expectedTilePath))
