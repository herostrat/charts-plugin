...
import { detectTypeFromName, escapeHtml, fmtBytes, fmtIso, isSupportedType, requiresMeta } from '../core/utils.js';
...
const selectLocalEntry = (ent: any) => {
...
export const updateLocalRegisterState = () => {
  ...
  if (!isSupportedType(t)) {
    if (registerBtn) registerBtn.disabled = true;
    setStatus(registerStatus, 'status is-warn', 'Unsupported type. Choose a supported detected type.');
    toggleMetaBox(localMetaBox, localMetaStatus, false);
    return;
  }
  ...
  if (needMeta) {
    if (!sel.isDir) {
      if (registerBtn) registerBtn.disabled = true;
      setStatus(registerStatus, 'status is-warn', 'Type "folder" requires selecting a directory.');
    ...
    setStatus(registerStatus, v.ok ? 'status' : 'status is-warn', v.ok ? 'Ready to register folder.' : 'Fill required metadata for folder import.');
    return;
  }
...
export const initLocal = (opts: { refreshJobs: () => Promise<void>; isSseConnected: () => boolean }) => {
  ...
  registerBtn?.addEventListener('click', async () => {
    ...
    try {
      const item: { filename: string; sourcePath: string; sizeBytes?: number; detectedType: string; metadata?: unknown } = {
        filename: sel.name,
        sourcePath: sel.path,
        sizeBytes: sel.size,
        detectedType: t
      ...
      const meta = getRequiredMeta(t, 'local', registerStatus, 'Missing required folder metadata.');
      if (meta === null) return;
      ...
      setStatus(registerStatus, 'status is-bad', `Register failed: ${e.message}`);
    } finally {
      if (registerBtn) registerBtn.disabled = true;
    }
  });
