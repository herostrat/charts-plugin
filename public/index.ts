...
(() => {
  ...
  sourceTabBtns.forEach((b) => b.addEventListener('click', () => setActiveSourceTab(b.dataset.tab || 'register')));
  ...
  const runSelfTests = () => {
    const assert = (cond: boolean, msg: string) => { if (!cond) console.warn('[selftest]', msg); };
